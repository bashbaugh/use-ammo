// Derived from https://github.com/osman-turan/ammo.js-typings
// MIT License
...
class AmmoJsTypeDefinitionsGenerator {
  ...
  private fixIDL(idl: string): string {
    return (
      idl
        // replace implements with includes
        .replace(/implements/g, 'includes')
        // replace float[] with sequence<float>
        .replace(/([a-zA-Z]+)\[\]/g, 'sequence<$1>')
    )
  }

  generate(webidlPath: string) {
    const idlStr = fs.readFileSync(webidlPath, { encoding: 'utf8' })

    const rootTypes = webidl2.parse(this.fixIDL(idlStr))
    this.start(rootTypes)
    // TODO find AmmoConfig type
    this.emitLines(
      '// This file is autogenerated from ammo.idl',
      '',
      'declare function Ammo(AmmoConfig: any): Promise<typeof Ammo>;',
      '',
...
fs.writeFileSync(path.resolve(__dirname, 'ammo.d.ts'), output)
